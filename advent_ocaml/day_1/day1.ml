...
let parse_line line =
  ...
  |> List.of_seq
  |> filter is_digit
  |> fun cs -> (hd cs, hd (rev cs))
  |> fun (f, l) -> int_of_string (String.make 1 f ^ String.make 1 l)
...
let parse_line_menhir line =
  let construct_int cs = 
    ...
    match int_of_string (string_of_int f ^ string_of_int l) with
    | i -> i
    | exception Failure s -> print_endline ("something weird happened: " ^ s); 0
  in
  Lexing.from_string line
  |> Parser.input Lexer.read
  |> construct_int

let parse_p2 line =
  let re = Str.regexp "[1-9]\\|one\\|two\\|three\\|four\\|five\\|six\\|seven\\|eight\\|nine" in
  let parse_digit s =
    match s with
    | "one" -> "1"
    | "two" -> "2"
...
(** [part_lr1 file] would be the answer to Day 1 Part 2, except it doesn't
    ...
    puzzle input *)
let part_lr1 file =
  process_file parse_line_menhir (open_in file)
  |> List.fold_left (+) 0
